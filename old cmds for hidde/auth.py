...
async def servers_check(bot: commands.AutoShardedBot): 
  ...
  for result in results: 
   if result['billing'].timestamp() < datetime.datetime.now().timestamp(): 
    ...
    await bot.get_channel(1136729223776710757).send(f"Leaving **{result['guild_id']}** since no one paid the monthly bill")
...
class Auth(commands.Cog): 
    ...
    async def subscriber_join(self, member: discord.Member): 
      if member.guild.id == 1136696949303681037:
       check = await self.bot.db.fetchrow("SELECT * FROM authorize WHERE buyer = $1", member.id)
       if check: await member.add_roles(member.guild.get_role(1136697621508001964))
    ...
    async def authorize(self, ctx: commands.Context, guildid: int=None, buyer: Union[discord.Member, discord.User]=None, offer: str=None): 
     if guildid is None or buyer is None: return await ctx.send_warning("command usage: {}authorize [server id] [buyer mention] [monthly/onetime]".format(ctx.clean_prefix)) 
     check = await self.bot.db.fetchrow("SELECT * FROM authorize WHERE guild_id = $1", guildid)
     if check is not None: return await ctx.send_warning(f"this server is already authorized. please use `{ctx.clean_prefix}transfer` to transfer a server authorization")
     
     if not offer.lower() in ["monthly", "onetime"]: return await ctx.send_warning("command usage: {}authorize [server id] [buyer mention] [monthly/onetime]".format(ctx.clean_prefix)) 
