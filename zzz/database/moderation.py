...
async def get_prefix(self, ctx):
        ...
        return (await self.bot.get_prefix(ctx.message))[2]
...
# Purge command group
    @commands.group(name="purge", invoke_without_command=True, description="Purge messages in the channel.")
    @commands.has_permissions(manage_messages=True)
    @commands.bot_has_permissions(manage_messages=True)
    async def purge(self, ctx, amount: int):
        """ A base command for purging messages. If no subcommand is provided, it will default to purging any messages. """
        if amount < 1:
            embed = discord.Embed(description="You need to specify a number greater than 0.", color=Colors.yellow)
            await ctx.send(embed=embed)
            return

        deleted = await ctx.channel.purge(limit=amount)
        embed = discord.Embed(description=f"Successfully deleted {len(deleted)} messages.", color=Colors.green)
        await ctx.send(embed=embed, delete_after=5)

    @purge.command(name="bots", description="Purge messages sent by bots within the specified number of messages.")
    async def purge_bots(self, ctx, amount: int):
        """ Subcommand to purge messages sent by bots. """
        def is_bot(m):
            return m.author.bot

        deleted = await ctx.channel.purge(limit=amount, check=is_bot)
        embed = discord.Embed(description=f"Successfully deleted {len(deleted)} bot messages.", color=Colors.green)
        await ctx.send(embed=embed, delete_after=5)
